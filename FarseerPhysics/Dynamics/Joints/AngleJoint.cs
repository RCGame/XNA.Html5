...
namespace FarseerPhysics.Dynamics.Joints
{
    ...
    public class AngleJoint : Joint
    {
        ...
        internal override void SolveVelocityConstraints(ref TimeStep step)
        {
            ...
            BodyA.AngularVelocity -= BodyA.InvI * Math.Sign(p) * Math.Min((float)Math.Abs(p), MaxImpulse);
            BodyB.AngularVelocity += BodyB.InvI * Math.Sign(p) * Math.Min((float)Math.Abs(p), MaxImpulse);
        }
