...
namespace FarseerPhysics.Dynamics.Joints
{
    ...
    public class AngleJoint : Joint
    {
        ...
        internal override void SolveVelocityConstraints(ref SolverData data)
        {
            ...
            data.velocities[indexA].w -= BodyA._invI * Math.Sign(p) * Math.Min((float)Math.Abs(p), MaxImpulse);
            data.velocities[indexB].w += BodyB._invI * Math.Sign(p) * Math.Min((float)Math.Abs(p), MaxImpulse);
        }
